            ...
            while (true) {
                ...
                if (intValueRM == this.rmNumber)
                {
                    ...
                    if (incomingMessage.has("server")) {
                        ...
                        {	
                        	seqNumber = CAReplica.getSeqNumber();
                            oldPort = CAReplica.getPort();
                            CAReplica.interrupt();
                            CAReplica = new UDPServer(oldPort,replicaToReplaceLocation,replacmentReplica,this.rmNumber, seqNumber, false);
                        }
                        else if(replicaToReplaceLocation.equals("US"))
                        {	
                        	seqNumber = USReplica.getSeqNumber();
                            oldPort = USReplica.getPort();
                            USReplica.interrupt();
                            USReplica = new UDPServer(oldPort,replicaToReplaceLocation,replacmentReplica,this.rmNumber, seqNumber, false);
                        }
                        else
                        {	
                        	seqNumber = UKReplica.getSeqNumber();
                            oldPort = UKReplica.getPort();
                            UKReplica.interrupt();
                            UKReplica = new UDPServer(oldPort,replicaToReplaceLocation,replacmentReplica, this.rmNumber, seqNumber, false);
                        }

                        System.out.println("Successful Replacment for RM #" + this.rmNumber + " with location " + replicaToReplaceLocation);

                        UDPServer newReplica = new UDPServer(oldPort,replicaToReplaceLocation,replacmentReplica, this.rmNumber, seqNumber, false);
                        newReplica.start();
                        newReplica.prinData();
                        //**********************************************************************
                    }
